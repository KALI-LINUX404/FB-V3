@echo off
title FIRST ZONE ADMIN - RESET GUEST
color 03
(Set LF=^
%Null%
)
...
:OldIDR
cls
CHOICE /C 123 /N /T 30 /D 3 /M "1. Auto Extract And Apply!LF!2. Input Old ID Manually!LF!3. Go Back!LF!%1
IF ERRORLEVEL 3 goto IDR
IF ERRORLEVEL 2 goto Manual
IF ERRORLEVEL 1 goto Automatic

:Manual
set "filePath=OldDeviceID.txt"
...
:Automatic
REM Read the third line from the file
set "lineNum=0"
for /f "usebackq delims=" %%a in ("%filePath%") do (
    set /a "lineNum+=1"
    if !lineNum! equ 3 (
        set "thirdLine=%%a"
        goto :ProcessThirdLine
    )
)

echo Third line not found in the file.
goto :EOF

:ProcessThirdLine
REM Extract the string after ":"
for /f "tokens=2 delims=:" %%b in ("!thirdLine!") do (
    set "extractedString=%%b"
)

REM Remove spaces from the extracted string
set "extractedString=!extractedString: =!"

echo Extracted String: !extractedString!

adb shell settings put secure android_id !extractedString!
adb shell content insert --uri content://settings/secure --bind name:s:android_id --bind value:s:!extractedString!
set h=%TIME:~0,2%
set m=%TIME:~3,2%
set s=%TIME:~6,2%
set time=%h%_%m%_%s%
...
echo %DMY% %time%>OldDeviceID.tmp
echo Previous ID : %OldID%>>OldDeviceID.tmp
echo Reapplied ID : !extractedString!>>OldDeviceID.tmp
echo =====================================================>>OldDeviceID.tmp
If exist OldDeviceID.txt Ren OldDeviceID.txt OldDeviceID.txtx
Copy OldDeviceID.tmp+OldDeviceID.txtx OldDeviceID.txt > nul
if exist OldDeviceID.txtx Del OldDeviceID.txtx
...
adb shell setprop ro.mac_address !extractedString!
adb shell setprop ro.product.device !extractedString!
adb shell setprop ro.product.brand !extractedString!
adb shell setprop ro.product.model !extractedString!
adb shell setprop ro.product.name !extractedString!
adb shell setprop ro.product.manufacturer !extractedString!
adb shell setprop ro.android_id !extractedString!
adb shell setprop net.hostname !extractedString!
adb shell setprop gaid !extractedString!
adb shell setprop android.device.id !extractedString!
adb shell setprop ro.serialno !extractedString!
adb shell setprop ro.runtime.firstboot !extractedString!
...
adb shell rm /data/app/%PackageName%-1/lib/arm/unity.so
adb shell rm /data/app/%PackageName%-1/lib/arm/ashax.so
adb shell rm /data/app/%PackageName%-1/lib/arm/libc++_shared.so
adb shell cp /data/share1/unity.so /data/app/%PackageName%-1/lib/arm
adb shell cp /data/share1/ashax.so /data/app/%PackageName%-1/lib/arm
...
adb shell rm -rf /data/data/%PackageName%/databases/*
adb shell rm -rf /data/data/%PackageName%/files/.system_android_l2
adb shell rm -rf /data/data/%PackageName%/files/loginidentifier.txt
adb shell rm -rf /data/data/%PackageName%/files/*.dat
adb shell rm -rf /storage/emulated/0/Android/data/%PackageName%/cache/*
adb shell rm -rf /storage/emulated/0/Android/data/%PackageName%/files/.system_android_l2
adb shell rm -rf /storage/emulated/0/Android/data/%PackageName%/files/loginidentifier.txt
adb shell rm -rf /storage/emulated/0/Android/data/%PackageName%/files/*.dat
